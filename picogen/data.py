    ...
    def load_data(
        ...
        with mp.Pool() as pool:
            for i, song in enumerate(
                tqdm(
                    pool.imap(self.load_data_map, map_args),
                    total=len(map_args),
                    dynamic_ncols=True,
                )
            ):
                ...
                tokenizer.events_to_midi(
                    song["events"], song["metadata"]["beat_per_bar"]
                ...
                tokenizer.events_to_midi(
                    song["ls_events"], song["metadata"]["beat_per_bar"]
                ).dump(song_dir / "ls_events.mid")
    ...
    def collate_fn(batch, tokenizer, max_seq_len):
        ...
        for song in batch:
            ...
            if len(mixed_ids) // (max_seq_len // 2) == 0:
                ...
                idx_from = randrange(0, len(mixed_ids) // (max_seq_len // 2)) * (
                    max_seq_len // 2
                )
            ...
            cond_mask = torch.BoolTensor(cond_mask)[:, None].expand(
                -1, tgt_seg.shape[-1]
            )
            ...
            pad_id = -1
            tgt_seg = F.pad(
                tgt_seg, (0, 0, 0, max_seq_len + 1 - len(tgt_seg)), "constant", pad_id
            )  # +1 for bos
            cond_mask = F.pad(
                cond_mask,
                (0, 0, 0, max_seq_len - cond_mask.shape[-2]),
                "constant",
                False,
            )

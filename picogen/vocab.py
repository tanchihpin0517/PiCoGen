...
class Vocab:
    def __init__(self, vocab_file, repr):
        ...
        self.metric_i2w = [
            ...
            or word.startswith("beat")
            or word.startswith("tempo")
            or word.startswith("chord")
        ]
        self.metric_w2i = {self.metric_i2w[i]: i for i in range(len(self.metric_i2w))}

        self.note_i2w = NoteList(
            [
                word
                for word in words
                if word.startswith("note")
                or word.startswith("shift")
                or word.startswith("pitch")
                or word.startswith("velocity")
                or word.startswith("duration")
            ]
        )
        self.note_w2i = NoteDict(
            {self.note_i2w[i]: i for i in range(len(self.note_i2w))}
        )
    ...
    def get_family(self, word):
        if word in self.spec_i2w:
            ...
            word.startswith("beat")
            or word.startswith("tempo")
            or word.startswith("chord")
        ):
            return "metric"
        elif (
            word.startswith("shift")
            or word.startswith("pitch")
            or word.startswith("velocity")
            or word.startswith("duration")
        ):
